...
const appointmentSchema = Joi.object({
  doctor: Joi.string().length(12).required(),
  patient: Joi.string().length(12).required(),
  date: Joi.string().length(12).required(),
  symptoms: Joi.array().required().items(
    Joi.string().max(300).required()
  )
})
